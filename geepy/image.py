...
def tassCapTransformation(img, satellite):
    ...
    tassCap = ee.Image(brightness).addBands(greenness).addBands(
        wetness).addBands(TC4).addBands(TC5).addBands(TC6)
...
def calcNDFI2(img, bandName='ndfi2'):
    ...
    ndfi2 = ee.Image.cat(
        gvnpv, soilcloud).normalizedDifference().multiply(100).add(100)
...
def calcNDFI3(img, bandName='ndfi3'):
    ...
    ndfi3 = ee.Image.cat(
        gvnpv, soilshade).normalizedDifference().multiply(100).add(100)
...
def calcFractions(img):
    ...
    # Unmixing data
    fractions = ee.Image(img).select(bandNames).unmix(
        endmembers).max(0).multiply(10000)
...
def randomForest(img, training, bands, ntrees=10, classBand='class'):
    ...
    classifier = ee.Classifier.smileRandomForest(
        ntrees).train(training, classBand, bands)
...
def minimumDistance(img, training, bands, classBand='class', metric='euclidean'):
    ...
    classifier = ee.Classifier.minimumDistance(
        metric).train(training, classBand, bands)
    classification = img.select(bands).classify(classifier)
...
def SVM(img, training, bands, classBand='class', kernelType='LINEAR'):
    ...
    classifier = ee.Classifier.svm(
        kernelType=kernelType).train(training, classBand, bands)
    classification = img.select(bands).classify(classifier)
...
def wekaCascadeKMeans(img, training, bands, minClusters=2, maxClusters=5):
    """
        ...
        Args:
            ...
            maxClusters:
    """
    classifier = ee.Clusterer.wekaCascadeKMeans(
        minClusters, maxClusters).train(training, bands)
    classification = img.select(bands).cluster(classifier)
